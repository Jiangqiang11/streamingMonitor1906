...
import com.jq.streamingMoitor.common.bean.{AnalyzeRule, BookRequestData, RequestType}
import com.jq.streamingMoitor.common.util.decode.MD5
import com.jq.streamingMoitor.common.util.decode.RequestDecoder
import com.jq.streamingMoitor.common.util.json.JsonParser
import com.jq.streamingMoitor.common.util.xml.xmlUtil
import com.jq.streamingMoitor.dataprocess.constants.FlightTypeEnum
...
object AnalyzeBookRequest {
  ...
  def analyzeBookRequest(
                          ...
                          analyzeRules: List[AnalyzeRule]
                        ): Option[BookRequestData] = {
    var resultData: BookRequestData = new BookRequestData
    //循环规则，匹配出符合数据的规则
    val matchedRules: List[AnalyzeRule] = analyzeRules.filter { rule =>
      //匹配请求方式和请求类型 - 预定
